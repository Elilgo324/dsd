...
class StochasticAdditiveLackPlanner(Planner):
    ...
    def plan(self, env: StochasticEnvironment):
        ...
        lb, rb, tb = env.left_border, env.right_border, env.top_border
        v = agents[0].v
        fv = robots[0].fv
        r = robots[0].d
        ...
        for i_wave, wave in enumerate(waves):
            ...
            new_agents = [StochasticAgent(
                Point(max(lb, min(rb, normal(loc=agent.x, scale=sigma_t(sigma, v * completion_time)))),
                      agent.y + v * completion_time), v, sigma) for agent in wave]

            new_env = StochasticEnvironment(robots=new_robots, agents=new_agents, top_border=tb,
                                            left_border=lb, right_border=rb)
