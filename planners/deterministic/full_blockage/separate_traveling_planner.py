    ...
    def plan(self, env: Environment) -> Tuple[Dict[BasicRobot, List[Point]], float, float, int]:
        ...
        for i_robot in range(len(robots)):
            ...
            for i_bucket in range(len(agents_buckets)):
                ...
                trp_per_h = {h: line_trpv(h, fv, bucket, makespan_per_h[h]) for h in H}

                def damage_score(h):
                    return trp_per_h[h]['damage'] + len(bucket) * makespan_per_h[h]
                ...
                trp_per_h[h_opt]['ys'] = [h_opt] + trp_per_h[h_opt]['ys']
                trp_per_h[h_opt]['t'] = makespan_per_h[h_opt] + trp_per_h[h_opt]['t']
                trp_data_per_robot_bucket[i_robot][i_bucket] = trp_per_h[h_opt]
